...
const transformRoomData = (roomData) => ({
    ...
    is_connected: roomData.is_connected === 1 || roomData.is_connected === true,
});
...
export const roomApi = {
  ...
  async getRoomsAndOffsets() {
    try {
      ...
      // TemperatureOffset und HumidityOffset zu den Räumen hinzufügen
      const updatedRooms = rooms.map((room) => ({
        ...room,
        temperatureOffset: offsets?.temperature_offset ?? 1,
        humidityOffset: offsets?.humidity_offset ?? 5,
      }));

      return { rooms: updatedRooms, offsets };
    } catch (error) {
